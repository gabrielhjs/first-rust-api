...
pub async fn create_user(
    ...
    Ok(
        ...
        {
            Ok(user) => web::HttpResponse::Created().json(ResponseData {
                id: user.id.to_string(),
                username: user.username,
            }),
            Err(error) => web::HttpResponse::BadRequest().json(ResponseError {
...
pub async fn delete_user(
    ...
    let count =
        diesel::delete(schema::users::table.filter(id.eq(Uuid::parse_str(&data.id).unwrap())))
            .execute(&conn)
